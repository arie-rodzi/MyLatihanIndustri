...
c.execute("SELECT id, minggu, aktiviti, disahkan_oleh FROM logbook WHERE pelajar_id=? ORDER BY minggu", (pelajar_id,))
entries = c.fetchall()
...
if not entries:
    ...
    for log_id, minggu, aktiviti, disahkan in entries:
        with st.expander(f"📘 Minggu {minggu}:"):
            st.markdown(f"<div style='text-align: left; font-size: 16px; font-weight: bold; color: #333;'>{aktiviti}</div>", unsafe_allow_html=True)
            if disahkan:
                st.success(f"✅ Telah disahkan oleh {disahkan}")
            else:
                if st.button(f"Sahkan Minggu {minggu}", key=f"sahkan_{log_id}"):
                    c.execute("UPDATE logbook SET disahkan_oleh=? WHERE id=?", (penyelia_id, log_id))
                    conn.commit()

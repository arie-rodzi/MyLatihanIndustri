...
# Semak sama ada penyelaras yang login
if st.session_state.get("user_role") != "penyalaras":
...
# Papar senarai pelajar yang belum diluluskan
c.execute("""
...
for pelajar_id, nama, status_lulus in pelajar_list:
    with st.expander(f"{nama} ({pelajar_id})"):
        ...
        if st.button(f"✅ Luluskan SLI-01 untuk {nama}", key=pelajar_id):
            c.execute("INSERT OR REPLACE INTO status_permohonan (pelajar_id, status_lulus, tarikh_lulus) VALUES (?, ?, ?)",
